...
from keys import RSAUsageError, ECDSAUsageError
import fileinput
...
def gen_ecdsa_p256(keyfile, passwd, export):
    new_key=keys.ECDSA256P1.generate()
    new_key.export_private(path=keyfile,passwd=passwd)
    if export:
      new_key.export_public(path=export)

def gen_ecdsa_p256_ssl(keyfile, passwd, export):
    new_key=keys.ECDSA256P1_SSL.generate()
    new_key.export_private(path=keyfile,passwd=passwd)
    if export:
      new_key.export_public(path=export)

def gen_sym_32(keyfile, passwd, export):
    new_key=keys.SYMKEY.generate()
    new_key.export_private(path=keyfile,passwd=passwd)
...
@click.option('--pad-sig', default=None, is_flag=True,
...
def sign(key, align, version, pad_sig, header_size, pad_header, slot_size, pad, confirm,
         ...
         security_counter, boot_record, otfdec, primary_only, clear):
    img = image.Image(version=decode_version(version), header_size=header_size,
                      ...
                      otfdec=otfdec,primary_only=primary_only)
    img.load(infile)
    key = load_key(key) if key else None
    ...
    img.create(key, enckey, dependencies, boot_record, clear=clear)
