...
def getprivbin(key, minimal):
    key = load_key(key)
    if key is None:
        print("Invalid passphrase")
    try:
        key.emit_private_bin(minimal)
    except (RSAUsageError, ECDSAUsageError, Ed25519UsageError,
            X25519UsageError) as e:
        raise click.UsageError(e)
...
def sign(key, public_key_format, align, version, pad_sig, header_size,
         ...
         rom_fixed,otfdec, primary_only, clear):
    ...
    img = image.Image(version=decode_version(version), header_size=header_size,
                      ...
                      security_counter=security_counter, otfdec=otfdec,primary_only=primary_only)
    img.load(infile)
    key = load_key(key) if key else None
