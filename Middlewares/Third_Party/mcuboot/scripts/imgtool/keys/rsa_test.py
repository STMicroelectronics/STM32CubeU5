...
class KeyGeneration(unittest.TestCase):
    ...
    def test_emit(self):
        ...
        for key_size in RSA_KEY_SIZES:
            ...
            self.assertIn(f"rsa{str(key_size)}_pub_key", ccode.getvalue())
            self.assertIn(f"rsa{str(key_size)}_pub_key_len", ccode.getvalue())

            rustcode = io.StringIO()
            k.emit_rust_public(rustcode)
            self.assertIn(f"RSA{str(key_size)}_PUB_KEY", rustcode.getvalue())
