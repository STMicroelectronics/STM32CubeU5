...
/** @defgroup STM32U575I_EVAL_OSPI OSPI
  * @{
  */

/* Exported variables --------------------------------------------------------*/
/** @addtogroup STM32U575I_EVAL_OSPI_NOR_Exported_Variables OSPI NOR Exported Variables
  * @{
  */
...
/* Private constants --------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
/** @defgroup STM32U575I_EVAL_OSPI_NOR_Private_Variables OSPI NOR Private Variables
  * @{
  */
...
/** @defgroup STM32U575I_EVAL_OSPI_NOR_Private_Functions OSPI NOR Private Functions
  * @{
  */
...
/** @addtogroup STM32U575I_EVAL_OSPI_NOR_Exported_Functions OSPI NOR Exported Functions
  * @{
  */
...
int32_t BSP_OSPI_NOR_Init(uint32_t Instance, BSP_OSPI_NOR_Init_t *Init)
{
  ...
  else
  {
    ...
    if (Ospi_Nor_Ctx[Instance].IsInitialized == OSPI_ACCESS_NONE)
    {
      ...
      if (MX25LM51245G_GetFlashInfo(&pInfo) != MX25LM51245G_OK)
      {
        return BSP_ERROR_PERIPH_FAILURE;
      }
      ...
      else if (OSPI_DLYB_Enable(Instance) != BSP_ERROR_NONE)
      {
...
int32_t BSP_OSPI_NOR_RegisterMspCallbacks(uint32_t Instance, BSP_OSPI_Cb_t *CallBacks)
{
  ...
  else
  {
    ...
    else if (HAL_OSPI_RegisterCallback(&hospi_nor[Instance], HAL_OSPI_MSP_DEINIT_CB_ID, CallBacks->pMspDeInitCb)
             != HAL_OK)
    {
      ret = BSP_ERROR_PERIPH_FAILURE;
    }
...
int32_t BSP_OSPI_NOR_Read(uint32_t Instance, uint8_t *pData, uint32_t ReadAddr, uint32_t Size)
{
  ...
  else
  {
    ...
    {
      if (MX25LM51245G_ReadSTR(&hospi_nor[Instance], Ospi_Nor_Ctx[Instance].InterfaceMode, MX25LM51245G_4BYTES_SIZE,
                               pData, ReadAddr, Size) != MX25LM51245G_OK)
      {
        ret = BSP_ERROR_COMPONENT_FAILURE;
      }
      else
      ...
      else
      {
        if (Ospi_Nor_Ctx[Instance].TransferRate == BSP_OSPI_NOR_STR_TRANSFER)
        {
          ...
          if (MX25LM51245G_PageProgram(&hospi_nor[Instance], Ospi_Nor_Ctx[Instance].InterfaceMode,
                                       MX25LM51245G_4BYTES_SIZE, (uint8_t *)data_addr, current_addr, current_size)
              != MX25LM51245G_OK)
          {
            ret = BSP_ERROR_COMPONENT_FAILURE;
          }
  ...
  else
  {
                                      ...
                                      Ospi_Nor_Ctx[Instance].TransferRate) != MX25LM51245G_OK)
    {
      ret = BSP_ERROR_COMPONENT_FAILURE;
                                     ...
                                     Ospi_Nor_Ctx[Instance].TransferRate, MX25LM51245G_4BYTES_SIZE, BlockAddress,
                                     BlockSize) != MX25LM51245G_OK)
    {
      ret = BSP_ERROR_COMPONENT_FAILURE;
    }
    else
...
/** @addtogroup STM32U575I_EVAL_OSPI_NOR_Private_Functions OSPI NOR Private Functions
  * @{
  */
