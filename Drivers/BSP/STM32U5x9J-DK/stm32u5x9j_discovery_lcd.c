...
int32_t BSP_LCD_Init(uint32_t Instance, uint32_t Orientation)
{
  ...
  else if ((Orientation == LCD_ORIENTATION_LANDSCAPE) || (Orientation == LCD_ORIENTATION_LANDSCAPE_ROT180))
  {
...
int32_t BSP_LCD_DisplayOn(uint32_t Instance)
{
  ...
  {
    /* Set the display on */
    if (HAL_DSI_ShortWrite(&hlcd_dsi, 0, DSI_DCS_SHORT_PKT_WRITE_P1, DSI_SET_DISPLAY_ON, 0x00) != HAL_OK)
    {
      status = BSP_ERROR_WRONG_PARAM;
    }
  }
...
int32_t BSP_LCD_DisplayOff(uint32_t Instance)
{
  ...
  {
    /* Set the display off */
    if (HAL_DSI_ShortWrite(&hlcd_dsi, 0, DSI_DCS_SHORT_PKT_WRITE_P1, DSI_SET_DISPLAY_OFF, 0x00) != HAL_OK)
    {
      status = BSP_ERROR_WRONG_PARAM;
    }
  }
...
int32_t BSP_LCD_DrawBitmap(uint32_t Instance, uint32_t Xpos, uint32_t Ypos, uint8_t *pBmp)
{
  ...
  else
  {
    ...
    /* Set the address */
    Address = (hlcd_ltdc.LayerCfg[0].FBStartAdress + (4U * ((Ypos * PIXEL_PER_LINE) + Xpos)));
    ...
    pbmp_tmp = pBmp + (index + (width * (height - 1U) * (bit_pixel / 8U)));

    /* Convert picture to ARGB8888 pixel format */
    for (index = 0; index < height; index++)
    {
      /* Pixel format conversion */
      (void)LCD_ConvertLineToARGB8888((uint32_t *)pbmp_tmp, (uint32_t *)Address, width, InputColorMode);

      /* Increment the source and destination buffers */
      Address += PIXEL_PER_LINE * BYTE_PER_PIXEL;
      pbmp_tmp -= width * (bit_pixel / 8U);
    }
  }
...
int32_t BSP_LCD_DrawHLine(uint32_t Instance, uint32_t Xpos, uint32_t Ypos, uint32_t Length, uint32_t Color)
{
  ...
  else
  {
    ...
    Xaddress = (hlcd_ltdc.LayerCfg[0].FBStartAdress + (4U * ((Ypos * PIXEL_PER_LINE) + Xpos)));

    /* Draw the line on LCD */
    for (i = 0; i < Length; i++)
    {
...
int32_t BSP_LCD_ReadPixel(uint32_t Instance, uint32_t Xpos, uint32_t Ypos, uint32_t *Color)
{
  ...
  {
    /* Read pixel on LCD */
    *Color = (*(__IO uint32_t *)(hlcd_ltdc.LayerCfg[0].FBStartAdress + (4U * ((Ypos * PIXEL_PER_LINE) + Xpos))));
  }
...
int32_t BSP_LCD_WritePixel(uint32_t Instance, uint32_t Xpos, uint32_t Ypos, uint32_t Color)
{
  ...
  {
    /* Write pixel on LCD */
    *(__IO uint32_t *)(hlcd_ltdc.LayerCfg[0].FBStartAdress + (4U * ((Ypos * PIXEL_PER_LINE) + Xpos))) = Color;
  }
...
__weak HAL_StatusTypeDef MX_GFXMMU_Init(GFXMMU_HandleTypeDef *hgfxmmu)
{
  ...
  hgfxmmu->Init.CachePrefetch.CacheLockBuffer     = GFXMMU_CACHE_LOCK_BUFFER0; /* NU */
  hgfxmmu->Init.CachePrefetch.CacheForce          = GFXMMU_CACHE_FORCE_ENABLE; /* NU */
  hgfxmmu->Init.CachePrefetch.OutterBufferability = GFXMMU_OUTTER_BUFFERABILITY_DISABLE;
  hgfxmmu->Init.CachePrefetch.OutterCachability   = GFXMMU_OUTTER_CACHABILITY_DISABLE;
...
/**
  ...
  * @retval HAL status.
  */
__weak HAL_StatusTypeDef MX_DMA2D_Init(DMA2D_HandleTypeDef *hdma2d, uint32_t Mode, uint32_t OffLine)
{
...
int32_t BSP_LCD_GFXMMU_RegisterDefaultMspCallbacks(uint32_t Instance)
{
  ...
  else
  {
    ...
    if (HAL_GFXMMU_RegisterCallback(&hlcd_gfxmmu, HAL_GFXMMU_MSPINIT_CB_ID, GFXMMU_MspInit) != HAL_OK)
    {
      status = BSP_ERROR_PERIPH_FAILURE;
    }
    else if (HAL_GFXMMU_RegisterCallback(&hlcd_gfxmmu, HAL_GFXMMU_MSPDEINIT_CB_ID, GFXMMU_MspDeInit) != HAL_OK)
    {
      status = BSP_ERROR_PERIPH_FAILURE;
    }
    else
...
int32_t BSP_LCD_GFXMMU_RegisterMspCallbacks(uint32_t Instance, BSP_LCD_GFXMMU_Cb_t *Callback)
{
  ...
  else
  {
    ...
    if (HAL_GFXMMU_RegisterCallback(&hlcd_gfxmmu, HAL_GFXMMU_MSPINIT_CB_ID, Callback->pMspGfxmmuInitCb) != HAL_OK)
    {
      status = BSP_ERROR_PERIPH_FAILURE;
    }
    else if (HAL_GFXMMU_RegisterCallback(&hlcd_gfxmmu, HAL_GFXMMU_MSPDEINIT_CB_ID, \
...
int32_t BSP_LCD_LTDC_RegisterDefaultMspCallbacks(uint32_t Instance)
{
  ...
  else
  {
    ...
    else if (HAL_LTDC_RegisterCallback(&hlcd_ltdc, HAL_LTDC_MSPDEINIT_CB_ID, LTDC_MspDeInit) != HAL_OK)
    {
      status = BSP_ERROR_PERIPH_FAILURE;
    }
    else
...
int32_t BSP_LCD_LTDC_RegisterMspCallbacks(uint32_t Instance, BSP_LCD_LTDC_Cb_t *Callback)
{
  ...
  else
  {
    ...
    /* Register MspInit/MspDeInit Callbacks */
    if (HAL_LTDC_RegisterCallback(&hlcd_ltdc, HAL_LTDC_MSPINIT_CB_ID, Callback->pMspLtdcInitCb) != HAL_OK)
    {
      status = BSP_ERROR_PERIPH_FAILURE;
    }
    else if (HAL_LTDC_RegisterCallback(&hlcd_ltdc, HAL_LTDC_MSPDEINIT_CB_ID, Callback->pMspLtdcDeInitCb) != HAL_OK)
    {
      status = BSP_ERROR_PERIPH_FAILURE;
    }
    else
...
int32_t BSP_LCD_DSI_RegisterDefaultMspCallbacks(uint32_t Instance)
{
  ...
  else
  {
    ...
    else if (HAL_DSI_RegisterCallback(&hlcd_dsi, HAL_DSI_MSPDEINIT_CB_ID, DSI_MspDeInit) != HAL_OK)
    {
      status = BSP_ERROR_PERIPH_FAILURE;
    }
    else
...
int32_t BSP_LCD_DSI_RegisterMspCallbacks(uint32_t Instance, BSP_LCD_DSI_Cb_t *Callback)
{
  ...
  else
  {
    ...
    /* Register MspInit/MspDeInit Callbacks */
    if (HAL_DSI_RegisterCallback(&hlcd_dsi, HAL_DSI_MSPINIT_CB_ID, Callback->pMspDsiInitCb) != HAL_OK)
    {
      status = BSP_ERROR_PERIPH_FAILURE;
    }
    else if (HAL_DSI_RegisterCallback(&hlcd_dsi, HAL_DSI_MSPDEINIT_CB_ID, Callback->pMspDsiDeInitCb) != HAL_OK)
    {
      status = BSP_ERROR_PERIPH_FAILURE;
    }
    else
...
int32_t BSP_LCD_DMA2D_RegisterDefaultMspCallbacks(uint32_t Instance)
{
  ...
  else
  {
    ...
    /* Register default MspInit/MspDeInit Callback */
    if (HAL_DMA2D_RegisterCallback(&hlcd_dma2d, HAL_DMA2D_MSPINIT_CB_ID, DMA2D_MspInit) != HAL_OK)
    {
      status = BSP_ERROR_PERIPH_FAILURE;
    }
    else if (HAL_DMA2D_RegisterCallback(&hlcd_dma2d, HAL_DMA2D_MSPDEINIT_CB_ID, DMA2D_MspDeInit) != HAL_OK)
    {
      status = BSP_ERROR_PERIPH_FAILURE;
    }
    else
...
int32_t BSP_LCD_DMA2D_RegisterMspCallbacks(uint32_t Instance, BSP_LCD_DMA2D_Cb_t *Callback)
{
  ...
  else
  {
    ...
    /* Register MspInit/MspDeInit Callbacks */
    if (HAL_DMA2D_RegisterCallback(&hlcd_dma2d, HAL_DMA2D_MSPINIT_CB_ID, Callback->pMspDma2dInitCb) != HAL_OK)
    {
      status = BSP_ERROR_PERIPH_FAILURE;
    }
    else if (HAL_DMA2D_RegisterCallback(&hlcd_dma2d, HAL_DMA2D_MSPDEINIT_CB_ID, Callback->pMspDma2dDeInitCb) != HAL_OK)
    {
      status = BSP_ERROR_PERIPH_FAILURE;
    }
    else
    ...
    if (status == BSP_ERROR_NONE)
    {
      ...
      else
      {
        ...
        if (status == BSP_ERROR_NONE)
        {
          ...
          {
            if (MX_LTDC_ConfigLayer(&hlcd_ltdc, LTDC_LAYER_1) != HAL_OK)
            {
              status = BSP_ERROR_PERIPH_FAILURE;
            }
          }
...
static int32_t LCD_DeInit(void)
{
  ...
  if (HAL_DSI_ShortWrite(&hlcd_dsi, 0, DSI_DCS_SHORT_PKT_WRITE_P0, DSI_ENTER_SLEEP_MODE, 0x0) != HAL_OK)
  {
    ErrorNumber++;
  }
...
static int32_t LCD_ConvertLineToARGB8888(uint32_t *pSrc, uint32_t *pDst, uint32_t xSize, uint32_t ColorMode)
{
...
#else
  /* Register the DMA2D MSP Callbacks */
  if (LcdDma2d_IsMspCbValid[0] == 0U)
  {
    if (BSP_LCD_DMA2D_RegisterDefaultMspCallbacks(0) != BSP_ERROR_NONE)
    {
      status = BSP_ERROR_PERIPH_FAILURE;
    }
  }
  ...
  if (status == BSP_ERROR_NONE)
  {
    ...
    if (MX_DMA2D_Init(&hlcd_dma2d, DMA2D_M2M_PFC, 0) != HAL_OK)
    {
      status = BSP_ERROR_PERIPH_FAILURE;
    }
    else
    {
      ...
      if (HAL_DMA2D_ConfigLayer(&hlcd_dma2d, 1) == HAL_OK)
      {
        if (HAL_DMA2D_Start(&hlcd_dma2d, (uint32_t)pSrc, (uint32_t)pDst, xSize, 1) == HAL_OK)
        {
          ...
          if (HAL_DMA2D_PollForTransfer(&hlcd_dma2d, 10) == HAL_OK)
          {
            if (HAL_DMA2D_DeInit(&hlcd_dma2d) == HAL_OK)
            {
#if (USE_HAL_DMA2D_REGISTER_CALLBACKS == 0)
              DMA2D_MspDeInit(&hlcd_dma2d);
#endif /* (USE_HAL_DMA2D_REGISTER_CALLBACKS == 0) */
            }
            else
            {
              status = BSP_ERROR_PERIPH_FAILURE;
...
static void DMA2D_MspInit(DMA2D_HandleTypeDef *hdma2d)
{
  ...
  HAL_NVIC_SetPriority(DMA2D_IRQn, BSP_LCD_DMA2D_IT_PRIORITY, 0);
  HAL_NVIC_EnableIRQ(DMA2D_IRQn);
}

/**
  * @brief  De-Initialize DMA2D MSP.
  * @param  hdma2d DMA2D handle
  * @retval None
  */
static void DMA2D_MspDeInit(DMA2D_HandleTypeDef *hdma2d)
