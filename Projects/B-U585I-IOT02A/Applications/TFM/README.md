...
The TFM application components are licenced by ST under Ultimate Liberty
...
<b>1. Build</b>
   ...
   2. Build TFM_Appli Secure project.
      ...
      It also produces the clear TFM Secure signed image for initial installation in
      TFM_Appli\Binary\tfm_s_init.bin,
      the encrypted TFM Secure signed image for download in
      TFM_Appli\Binary\tfm_s_enc_sign.bin
      and the clear TFM Secure signed image for download in
      TFM_Appli\Binary\tfm_s_sign.bin.
   ...
   3. Build TFM_Appli Non Secure project.
      ...
      This step creates the TFM Application Non Secure binary:
            ...
            MDK-ARM:      TFM_Appli\MDK-ARM\B-U585I-IOT02A_NS\Exe\Project.bin
            STM32CubeIDE: TFM_Appli\STM32CubeIDE\NonSecure\Release\TFM_Appli_NonSecure.bin
      It also produces the clear TFM Non Secure signed image for initial installation in
      TFM_Appli\Binary\tfm_ns_init.bin,
      the encrypted TFM Non Secure signed image for download in
      TFM_Appli\Binary\tfm_ns_enc_sign.bin
      and the clear TFM Non Secure signed image for download in
      TFM_Appli\Binary\tfm_ns_sign.bin.
   ...
   4. Build TFM_Loader Secure project.
      ...
      This step creates the TFM loader Secure binary:
            ...
            MDK-ARM:      TFM_Loader\MDK-ARM\B-U585I-IOT02A_S\Exe\Project.bin
   ...
   5. Build TFM_Loader Non Secure project.
      This step creates the TFM loader Non Secure binary:
            ...
            MDK-ARM:      TFM_Loader\MDK-ARM\B-U585I-IOT02A_NS\Exe\Project.bin
...
<b>7. Use User App</b>
   ...
   1. Press 1 to enter test protections menu
      ...
      - Test Protection (#1): test unauthorized accesses to Secure side from
        ...
        side. Several successives unauthorized accesses to different areas are
        performed, leading to successives reset when fault occurs.
   ...
   3. Press 3 to enter new firmware image menu
      ...
      It is possible to download a new Secure image or Non Secure image or both.
      - Download Secure Image (#2): Download encrypted signed TFM_Appli Secure image
        (TFM_Appli\Binary\tfm_s_enc_sign.bin) or clear signed
        TFM_Appli Secure image (TFM_Appli\Binary\tfm_s_sign.bin)
      - Download NonSecure Image (#3): Download encrypted signed TFM_Appli Non Secure image
        (TFM_Appli\Binary\tfm_ns_enc_sign.bin) or clear signed
        TFM_Appli Non Secure image (TFM_Appli\Binary\tfm_ns_sign.bin)
...
<b>9. Configuration</b>
   ...
   *  Number of images
      ...
      By default, 2 firmware images are managed with 2 distincts signatures:
      1 image for NS binary and 1 image for S binary.
      It is possible to assemble NS and S binaries into 1 image
      with 1 single signature, with MCUBOOT_IMAGE_NUMBER define in
      ...
      Note: When 2 images are selected, it is possible to download separately
      a new Secure image or Non Secure image or both.
      When 1 image is selected, it is possible to download a single new image.
