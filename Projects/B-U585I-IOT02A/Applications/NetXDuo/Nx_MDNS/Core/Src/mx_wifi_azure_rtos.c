...
UINT mx_wifi_thread_init(TX_THREAD * thread_ptr, CHAR *name_ptr, VOID (*entry_function)(ULONG), VOID* entry_input, ULONG stack_size, UINT priority)
{
  ...
  return tx_thread_create(
    thread_ptr,
    name_ptr,
    entry_function, (ULONG) entry_input,
    stack, stack_size,
    priority, priority,
    TX_NO_TIME_SLICE,
    TX_AUTO_START);
...
UINT mx_wifi_fifo_init(TX_QUEUE * queue_ptr, CHAR *name_ptr, ULONG size)
{
  ...
  return tx_queue_create(
    ...
    TX_1_ULONG,
    queue_start,
	size * TX_1_ULONG * sizeof(ULONG)); //reetika
}
