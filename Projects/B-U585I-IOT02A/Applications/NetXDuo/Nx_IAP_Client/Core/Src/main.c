...
int main(void)
{
  ...
  {
    {
      ...
      HAL_UART_Transmit(&huart1, (uint8_t*)welcome, welcome_size, 0xFFFF);
    }
  }
  ...
  /* LED6 and LED7 are already initialized. */
  /* Key Button is already initialized. */

  /* Test if Key Tamper-button on B-U585I-IOT02A Board is not pressed. */
  if (HAL_GPIO_ReadPin(USER_BUTTON_GPIO_Port, USER_BUTTON_Pin) == 0x00)
  {
    ...
    if ((stack_pointer & 0x2FFFE000) == 0x20002000)
    {
...
/* USER CODE END MX_GPIO_Init_1 */
  ...
  HAL_GPIO_WritePin(GPIOH, LED6_Pin|LED7_Pin, GPIO_PIN_SET);
  ...
  /*Configure GPIO pins : LED6_Pin LED7_Pin */
...
{
  /* LED6: LED6 : RED. */
  uint16_t led_pin = LED6_Pin;
  GPIO_TypeDef *led_gpio_port = LED6_GPIO_Port;
  uint32_t delay = 300;
