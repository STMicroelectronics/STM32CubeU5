...
The **AppMQTTClientThread**, once started:
  ...
  + Connects mqtt_client to the online MQTT broker `broker.hivemq.com` using TLS on port 8883; a **tls_setup_callback** sets all TLS parameters, including SNI and hostname verification.

          See note 4 below for x509 certificate setup.
  
  + mqtt_client subscribes on a predefined topic TOPIC_NAME  "Temperature" with a Quality Of Service QOS1
...
####  <b>Expected success behavior</b>
 ...
 + Connection's information are printed on the HyperTerminal (broker's name, mqtt port, topic and messages received)
  ...
  ```
MQTT client connected to broker < broker.hivemq.com > at PORT 8883 :
...
1. To make an encrypted connection with the MQTT server, follow these steps to add an x509 certificate to the _mqtt_client_ for server authentication:

  - The default broker is `broker.hivemq.com` on TLS port `8883`.

  - For HiveMQ, this application uses the Starfield Services Root CA G2 (`starfield_services_root_ca_g2.cert.h`). If the file is missing, generate it with the following commands:

    1. Print the broker certificate chain:
      ```
      openssl s_client -showcerts -servername broker.hivemq.com -connect broker.hivemq.com:8883 < NUL
      ```

    2. From that output, copy the PEM block for **Starfield Services Root Certificate Authority - G2** into a new file named `starfield_services_root_ca_g2.pem` (including `-----BEGIN CERTIFICATE-----` and `-----END CERTIFICATE-----`).

    3. Convert PEM to DER:
      ```
      openssl x509 -in starfield_services_root_ca_g2.pem -outform DER -out starfield_services_root_ca_g2.der
      ```

    4. Convert DER to C header:
      ```
      xxd -i starfield_services_root_ca_g2.der > starfield_services_root_ca_g2.cert.h
      ```

  - Place `starfield_services_root_ca_g2.cert.h` in: `NetXDuo/App`.

  - Set `#define MQTT_TLS_CA_CERT_FILE "starfield_services_root_ca_g2.cert.h"` in `app_netxduo.h`.

  - This application uses a single trusted root certificate: `starfield_services_root_ca_g2.cert.h`.

  - TLS certificate parsing in this application relies on the extra TLS buffers configured in `app_netxduo.c`.
