...
The **AppUDPThread**, once started:
  ...
  + Creates a <i>UDP</i> server socket

  + Listen indefinitely on new client connections.

  + As soon as a new connection is established, the <i>UDP</i> server socket starts receiving data packets from the client and prints the data on the HyperTerminal, then resend the same packet to the client.
...
In order to make the program work, you must do the following :
 ...
 - run the [echotool](https://github.com/PavelBansky/EchoTool/releases/tag/v1.5.0.0) utility on a windows console as following:
       ...
       c:\> echotool.exe  <the board IP address> /p udp  /r  <DEFAULT_PORT> /n 10 /d "Hello World"
