...
int main(void)
{
  ...
  /* While the SMBUS in reception process, user can transmit data through
     "aTxBuffer" buffer */
  while(HAL_SMBUS_Master_Transmit_IT(&hsmbus2, (uint16_t)SMBUS_ADDRESS, (uint8_t*)aTxBuffer, TXBUFFERSIZE + SMBUS_PEC_SIZE, SMBUS_FIRST_AND_LAST_FRAME_WITH_PEC)!= HAL_OK)
  {
    /* Error_Handler() function is called when Timeout error occurs.
       When Acknowledge failure occurs (Slave don't acknowledge its address)
       Master restarts communication */
    if (HAL_SMBUS_GetError(&hsmbus2) == HAL_SMBUS_STATE_MASTER_BUSY_TX)
    {
      Error_Handler();
    }
  }
  ...
  while(HAL_SMBUS_Master_Receive_IT(&hsmbus2, (uint16_t)SMBUS_ADDRESS, (uint8_t *)aRxBuffer, RXBUFFERSIZE + SMBUS_PEC_SIZE , SMBUS_FIRST_AND_LAST_FRAME_WITH_PEC) != HAL_OK)
  {
    /* Error_Handler() function is called when Timeout error occurs.
       When Acknowledge failure occurs (Slave don't acknowledge it's address)
       Master restarts communication */
    if (HAL_SMBUS_GetError(&hsmbus2) != HAL_SMBUS_ERROR_ACKF)
    {
      Error_Handler();
    }
  }
  while (hsmbus2.State == HAL_SMBUS_STATE_MASTER_BUSY_RX);
