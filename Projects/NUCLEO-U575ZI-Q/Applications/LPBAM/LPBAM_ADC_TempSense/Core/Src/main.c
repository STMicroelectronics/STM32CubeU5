...
int main(void)
{
  ...
  MX_TempSens_MultiThres_DeInit();
   ...
   /* Check that the ADC_Buffer contains a value greater then the threshold1 */
  if (ADC_Buffer_Check(Threshold1_Data_Buffer, 512, 1005) == 0U)
  {
    Error_Handler();
  }

  /* Check that the ADC_Buffer contains a value greater then the threshold2 */
...
static void MX_LPDMA1_Init(void)
{
  ...
  if (HAL_DMA_ConfigChannelAttributes(&handle_LPDMA1_Channel1, DMA_CHANNEL_PRIV) != HAL_OK)
  {
    Error_Handler();
  }
...
/**
  * @brief Verify whetherthe buffer has a value greater then the threshold or not.
