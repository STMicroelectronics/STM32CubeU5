...
/* LPBAM scenario buffers declaration */
uint32_t CompOutput_Buffer[OUTPUT_BUFFER_SIZE];

/* Expected read values */
uint32_t Expected_CompOutput_Buffer[OUTPUT_BUFFER_SIZE] = {INPUTMINUS_ABOVE_INPUTPLUS, INPUTMINUS_BELOW_INPUTPLUS,
...
int main(void)
{
  ...
  CompOutput_Data_Processing(CompOutput_Buffer, OUTPUT_BUFFER_SIZE);

  /* Compare read values to expected read values */
  if (Buffercmp(CompOutput_Buffer, Expected_CompOutput_Buffer, OUTPUT_BUFFER_SIZE) != 0U)
  {
    Error_Handler();
  }
...
static void CompOutput_Data_Processing(uint32_t* pBuffer, uint16_t BufferLength)
{
  while (BufferLength--)
  {
    /* Store the comparator output level */
