...
uint8_t Transmit_Buffer_1[65] = "LPBAM_LPUART_Two_Boards_Communication_LowPower_Mode_Packet_Size_8";
uint8_t Transmit_Buffer_2[65] = "LPBAM_LPUART_Two_Boards_Communication_LowPower_Mode_Packet_Size_4";

/* Buffer used for reception */
uint8_t Receive_Buffer[130];

DMA_HandleTypeDef *LPBAM_TxRx_SendByPack_DMAHandlers[2];
...
int main(void)
{
  ...
  /* LPBAM TxRx application SendByPack scenario init */
  MX_TxRx_SendByPack_Init();

  /* LPBAM TxRx application SendByPack scenario build */
  MX_TxRx_SendByPack_Build();

  /* Set DMA handlers */
  LPBAM_TxRx_SendByPack_DMAHandlers[0U] = &handle_LPDMA1_Channel0;
  LPBAM_TxRx_SendByPack_DMAHandlers[1U] = &handle_LPDMA1_Channel1;

  /* LPBAM TxRx application SendByPack scenario link */
  MX_TxRx_SendByPack_Link(LPBAM_TxRx_SendByPack_DMAHandlers[0U]);
  ...
  /* Wait for next push button press to stop the LPBAM TxRx application SendByPack scenario */
  while (PushButtonState == PB_NOT_PRESSED);

  /* Turn off LED2 */
  BSP_LED_Off(LED2);

  /* LPBAM TxRx application SendByPack scenario start */
  MX_TxRx_SendByPack_Start(LPBAM_TxRx_SendByPack_DMAHandlers[0U]);

  /* Enter Stop2 mode */
  Enter_Stop2_Mode();

  /* LPBAM TxRx application SendByPack scenario stop */
  MX_TxRx_SendByPack_Stop(LPBAM_TxRx_SendByPack_DMAHandlers[0U]);

  /* LPBAM TxRx application SendByPack scenario unlink */
  MX_TxRx_SendByPack_UnLink(LPBAM_TxRx_SendByPack_DMAHandlers[0U]);

  /* LPBAM TxRx application SendByPack scenario de-init */
  MX_TxRx_SendByPack_DeInit();
  ...
  /* Wait for next push button press to stop the LPBAM DAC app scenario */
  while (PushButtonState == PB_NOT_PRESSED);
  ...
  if (HAL_UART_Receive(&hlpuart1, (uint8_t *)Receive_Buffer, COUNTOF(Receive_Buffer), 0x1FFFFFF) != HAL_OK)
  {
    Error_Handler();
  }
  ...
  if (HAL_UART_Transmit(&hlpuart1, (uint8_t*)Receive_Buffer, COUNTOF(Receive_Buffer), 5000)!= HAL_OK)
  {
    Error_Handler();
  }
#endif /* TRANSMITTER_BOARD */
  ...
  if((Buffercmp((uint8_t*)Transmit_Buffer_1, (uint8_t*)Receive_Buffer, (COUNTOF(Transmit_Buffer_1) - 1U)) != 0))
  {
    Error_Handler();
  }

  /* Compare the sent and received buffer */
  if((Buffercmp((uint8_t*)Transmit_Buffer_2, &Receive_Buffer[COUNTOF(Transmit_Buffer_1)], COUNTOF(Transmit_Buffer_2)) != 0))
  {
    Error_Handler();
  }
