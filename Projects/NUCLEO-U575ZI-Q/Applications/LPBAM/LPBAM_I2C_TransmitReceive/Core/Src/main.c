...
uint8_t aTxBuffer1[TX_BUFFER1_SIZE] =
" ############### LPBAM I2C Application ############### ";

uint8_t aTxBuffer2[TX_BUFFER2_SIZE] =
...
int main(void)
{
  ...
  /* Wait for next push button press to stop the LPBAM Master application ReloadSeq scenario */
  ...
  if (Buffercmp(aTxBuffer2, aRxBuffer2, TX_BUFFER2_SIZE) != 0U)
  {
    Error_Handler();
  }
  ...
  /* LPBAM Slave application init */
  MX_Slave_Init();

  /* LPBAM Slave application ReloadSeq scenario init */
  MX_Slave_ReloadSeq_Init();

  /* LPBAM Slave application ReloadSeq scenario build */
  MX_Slave_ReloadSeq_Build();

  /* LPBAM Slave application ReloadSeq scenario link */
  MX_Slave_ReloadSeq_Link(&handle_LPDMA1_Channel0);

  /* LPBAM Slave application ReloadSeq scenario start */
  MX_Slave_ReloadSeq_Start(&handle_LPDMA1_Channel0);

  /* Enter Stop2 mode */
  Enter_Stop2_Mode();

  /* LPBAM Slave application ReloadSeq scenario stop */
  MX_Slave_ReloadSeq_Stop(&handle_LPDMA1_Channel0);

  /* LPBAM Slave application ReloadSeq scenario unlink */
  MX_Slave_ReloadSeq_UnLink(&handle_LPDMA1_Channel0);

  /* LPBAM Slave application ReloadSeq scenario de-init */
  MX_Slave_ReloadSeq_DeInit();

  /* Compare aRxBuffer1 with expected content */
  if (Buffercmp(aTxBuffer1, aRxBuffer1, TX_BUFFER1_SIZE) != 0U)
  {
    Error_Handler();
  }
