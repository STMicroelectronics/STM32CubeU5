...
/* Configure the peripherals common clocks */
  ...
  /* Display title */
  UTIL_LCD_FillRect(0, 190, LCD_WIDTH, 80, UTIL_LCD_COLOR_BLUE);
  UTIL_LCD_DisplayStringAt(0, 200, (uint8_t *) "LCD_DSI_ULPM_DataClock", CENTER_MODE);
  UTIL_LCD_DisplayStringAt(0, 240, (uint8_t *) " example ", CENTER_MODE);
  ...
  /* Copy texture to be displayed on LCD from Flash to LCD_FRAME_BUFFER */
  CopyPicture((uint32_t *)&image_320x240_argb8888, (uint32_t *)LCD_FRAME_BUFFER, 67, 140, IMAGE_WIDTH, IMAGE_HEIGHT);

  UTIL_LCD_SetFont(&Font20);
  UTIL_LCD_FillRect(0, 40, LCD_WIDTH, 60, UTIL_LCD_COLOR_BLUE);
  UTIL_LCD_DisplayStringAt(0, 50, (uint8_t *) "Press User-button", CENTER_MODE);
  UTIL_LCD_DisplayStringAt(0, 70, (uint8_t *) "to enter ULPM", CENTER_MODE);
  /* USER CODE END 2 */
  ...
  while (1)
  {
    if (CheckForUserInput() > 0)
    {
      ...
      UTIL_LCD_DisplayStringAt(0, 50, (uint8_t *) "Enter ULPM", CENTER_MODE);
      UTIL_LCD_DisplayStringAt(0, 70, (uint8_t *) "Switch-Off LCD 6 seconds", CENTER_MODE);

      HAL_Delay(2000);

      /* Display Off with ULPM management Data and clock lane integrated */
      if(HAL_DSI_ShortWrite(&hdsi, 0, DSI_DCS_SHORT_PKT_WRITE_P1, DSI_SET_DISPLAY_OFF, 0x00) != HAL_OK)
      {
        Error_Handler();
      }
      ...
      /* This means : laneByteClock is derived from PLL3.PLL3P : ie ck_plldsi @ 62.5 MHz = 160 MHz / PLLP= 8 */
      /* DSI State machine start and wait until ULPM state for Data and Clock lanes, then when reached, switch off PLL DPHY */
      if(HAL_DSI_EnterULPM(&hdsi) != HAL_OK)
      {
        Error_Handler();
      }
      ...
      UTIL_LCD_DisplayStringAt(0, 50, (uint8_t *) "   ULPM   ", CENTER_MODE);
      UTIL_LCD_DisplayStringAt(0, 70, (uint8_t *) "   Exited with success   ", CENTER_MODE);
      ...
      if(HAL_DSI_ExitULPM(&hdsi) != HAL_OK)
      {
        Error_Handler();
      }
      ...
      UTIL_LCD_SetTextColor(UTIL_LCD_COLOR_WHITE);
      UTIL_LCD_DisplayStringAt(0, 50, (uint8_t *) "Press User-button", CENTER_MODE);
      UTIL_LCD_DisplayStringAt(0, 70, (uint8_t *) "to enter again ULPM", CENTER_MODE);
     }
    /* USER CODE END WHILE */
...
static uint8_t CheckForUserInput(void)
{
  if(BSP_PB_GetState(BUTTON_USER) == GPIO_PIN_RESET)
  {
    ...
    do
    {
      /* Prepare area to display frame number on LCD */
      ...
      UTIL_LCD_DisplayStringAt(0, 400, (uint8_t *)str_display, CENTER_MODE);
