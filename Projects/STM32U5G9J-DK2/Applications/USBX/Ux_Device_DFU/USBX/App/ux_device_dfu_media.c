...
/* Bank 1 Base @ of Page 127, 8 Kbytes */
#define BANK_1_ADDR_FLASH_PAGE_127   ((uint32_t)0x080FE000)
/* Base Bank 2 @ of Page 127, 8 Kbytes */
#define BANK_2_ADDR_FLASH_PAGE_127   ((uint32_t)0x081FE000)

/* End Bank 1 @ of user Flash area */
#define BANK_1_FLASH_USER_END_ADDR   (BANK_1_ADDR_FLASH_PAGE_127 + FLASH_PAGE_SIZE - 1)
/* End Bank 2 @ of user Flash area */
#define BANK_2_FLASH_USER_END_ADDR   (BANK_2_ADDR_FLASH_PAGE_127 + FLASH_PAGE_SIZE - 1)
  ...
  while (1)
  {
    ...
    if (status == TX_SUCCESS)
    {
      ...
      else
      {
        ...
        for (Block_index = 0; Block_index < ux_dfu_download.wlength; Block_index += 16)
        {
...
static uint16_t DFU_Erase(uint32_t Address)
{
  ...
  /* Get the number of pages of specified bank */
  if ( BankNumber == FLASH_BANK_1)
  {
    /* Get the number of pages to erase from 1st page */
    NbOfPages = GetPage(BANK_1_FLASH_USER_END_ADDR) - FirstPage + 1;
  }
  else
  {
    /* Get the number of pages to erase from 1st page */
    NbOfPages = GetPage(BANK_2_FLASH_USER_END_ADDR) - FirstPage + 1;
  }
...
static uint32_t GetBank(uint32_t Addr)
{
  /* Check if given address is in bank 1 or 2 */
  if (Addr < (FLASH_BASE + FLASH_BANK_SIZE))
  {
    return FLASH_BANK_1;
  }
