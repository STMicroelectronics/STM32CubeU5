...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
    /* check if WKUP push-button has been pressed to switch clock config */
    if (SwitchClock != RESET)
    {
      SwitchSystemClock();
    }

    /* Toggle LED5 in an infinite loop */
    BSP_LED_Toggle(LED5);
    HAL_Delay(100);
  }
...
static void SwitchSystemClock(void)
{
  ...
  else if (__HAL_RCC_GET_PLL_OSCSOURCE() == RCC_PLLSOURCE_MSI)
  {
    ...
    /* Set SYSCLK frequency to 160000000 Hz, coming from the PLL which is clocked by HSE  */
