...
char Tx1Buffer[] = "**STM32U575I EEPROM****READ AND WRITE**";
...
static void EEPROM_SetHint(void)
{
  ...
  UTIL_LCD_DisplayStringAt(0, 45, (uint8_t*)"EEPROM Features, use USER button", CENTER_MODE);
...
static void EEPROM_Show_Feature(uint8_t feature)
{
  ...
  case 0:
    ...
    if(BSP_EEPROM_IsDeviceReady(0) == BSP_ERROR_NONE)
    {
      ...
      if (BSP_EEPROM_ReadBuffer(0, (uint8_t*)RxBuffer, EEPROM_READ_ADDRESS1, NumDataRead) != BSP_ERROR_NONE)
      {
        ...
        UTIL_LCD_DisplayStringAt(0, 145, (uint8_t*)"Press again USER button", CENTER_MODE);
      ...
      UTIL_LCD_DisplayStringAt(0, 190, (uint8_t*)"Press USER button", CENTER_MODE);
  ...
  case 1:
    ...
    if(BSP_EEPROM_IsDeviceReady(0) == BSP_ERROR_NONE)
    {
      ...
      UTIL_LCD_DisplayStringAt(0, 115, (uint8_t*)"wait until finish writing process ... ", CENTER_MODE);
      ...
      if(TransferStatus1 != FAILED)
      {
        ...
        UTIL_LCD_DisplayStringAt(0, 130, (uint8_t*)"in the current EEPROM selected:", CENTER_MODE);
        UTIL_LCD_DisplayStringAt(0, 160, (uint8_t*)Tx2Buffer, CENTER_MODE);
        UTIL_LCD_DisplayStringAt(0, 180, (uint8_t*)"READ/WRITE SUCCESS", CENTER_MODE);
      }
      else
      {
        UTIL_LCD_SetTextColor(UTIL_LCD_COLOR_RED);
    ...
    else
    {
      ...
      UTIL_LCD_DisplayStringAt(0, 145, (uint8_t*)"Press USER button to end test", CENTER_MODE);
    }
    break;
  case 2:
    if(BSP_EEPROM_IsDeviceReady(0) == BSP_ERROR_NONE)
