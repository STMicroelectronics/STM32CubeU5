...
/**
  ...
  * @brief   This example describes how to configure and use TSC in interrupt mode
  *          through the STM32U5xx HAL API.
  ******************************************************************************
  * @attention
  *
...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define TSCx_TS1_MAXTHRESHOLD 1900 /* This value can be modified, board layout dependency */
/* USER CODE END PD */
...
__IO uint32_t TSCAcquisitionValue;

TSC_IOConfigTypeDef IoConfig;

uint8_t TSCAcquisitionValueStr[256] = {0};
...
int main(void)
{
  ...
  UTIL_LCD_FillRect(0, 150, 320, 50, UTIL_LCD_COLOR_WHITE);
  UTIL_LCD_SetTextColor(UTIL_LCD_COLOR_BLUE);
  UTIL_LCD_SetBackColor(UTIL_LCD_COLOR_WHITE);
  UTIL_LCD_DisplayStringAt(0, 140, (uint8_t *)"Threshold value is indicative", CENTER_MODE);
  UTIL_LCD_DisplayStringAt(0, 155, (uint8_t *)"and may need to be adjusted!", CENTER_MODE);
  UTIL_LCD_DrawHLine(0, 210, 320, UTIL_LCD_COLOR_BLUE);
...
static void Display_DemoDescription(void)
{
  ...
  UTIL_LCD_DisplayStringAt(0, 180, (uint8_t *)"BasicAcquisition Interrupt", CENTER_MODE);
...
/**
  ...
  * @retval None
  */
void HAL_TSC_ConvCpltCallback(TSC_HandleTypeDef* TscHandle)
{
  ...
  if (HAL_TSC_GroupGetStatus(&htsc, TSC_GROUP4_IDX) == TSC_GROUP_COMPLETED)
  {
    ...
    TSCAcquisitionValue = HAL_TSC_GroupGetValue(&htsc, TSC_GROUP4_IDX);

    /* Display info */
    sprintf((char*)TSCAcquisitionValueStr, "Acquisition Value: %3u",(unsigned int)TSCAcquisitionValue);
    UTIL_LCD_DisplayStringAt(0, 180, (uint8_t *)TSCAcquisitionValueStr, CENTER_MODE);

    /* Note: Show the touch activity on LEDs.
       The threshold values are indicative and may need to be adjusted */
    if (TSCAcquisitionValue < TSCx_TS1_MAXTHRESHOLD)
      {
        /* TS1 touch detected */
        BSP_LED_On(LED5);
        BSP_LED_Off(LED6);
