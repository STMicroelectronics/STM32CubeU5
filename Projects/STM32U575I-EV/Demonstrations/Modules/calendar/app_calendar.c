...
static void Calendar_TimeShow(void)
{
  ...
  BSP_LCD_FillRGBRect(0, (uint32_t)xpos, (uint32_t)ypos, (uint8_t *)IconWatch, 128, 128);
...
static void Calendar_DateDisplay(uint8_t Year, uint8_t Month, uint8_t Day)
{
  ...
  BSP_LCD_GetXSize(0, &pXSize);
  UTIL_LCD_FillRect(0, 0, pXSize, Font20.Height,UTIL_LCD_COLOR_ST_BLUE_DARK);
  UTIL_LCD_FillRect(0, LINE(1), pXSize, Font20.Height, UTIL_LCD_COLOR_ST_BLUE_DARK);
  /* Set the Text Color */
  UTIL_LCD_SetTextColor(UTIL_LCD_COLOR_WHITE);
  ...
  sprintf((char*)linedisplay," WEEK: %02lu   DAY: %03lu    ", (unsigned long)wn, (unsigned long)dc);
...
static void Calendar_AlarmSet(void)
{
  ...
  Calendar_ClearMenuLines();
    ...
    BSP_LCD_FillRGBRect(0, (uint32_t)xpos, (uint32_t)ypos, (uint8_t *)IconAlarm, 128, 128);
  /* PreAdjust Time */
...
static void Calendar_AlarmPreAdjust(void)
{
  ...
  /* Switch on Green Led used as Alarm indicator */
