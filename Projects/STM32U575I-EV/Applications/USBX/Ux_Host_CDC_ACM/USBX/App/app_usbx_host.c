...
UINT MX_USBX_Host_Init(VOID *memory_ptr)
{
 ...
 /* Store byte_pool into ux_app_byte_bool */
  ...
  /* Allocate the stack for cdc_acm_recieve thread.  */
  if (tx_byte_allocate(byte_pool, (VOID **) &pointer,
  ...
  if (tx_thread_create(&cdc_acm_recieve_thread, "cdc_acm_recieve_thread", cdc_acm_recieve_app_thread_entry, 0,
  ...
  /* Allocate the stack for cdc_acm_send thread .  */
  if (tx_byte_allocate(byte_pool, (VOID **) &pointer,
  ...
  if (tx_thread_create(&cdc_acm_send_thread, "cdc_acm_send_thread", cdc_acm_send_app_thread_entry, 0,
...
void  ucpd_app_thread_entry(ULONG arg)
{
  ...
  while (1)
  {
    ...
    /* Check if received message equal to USB_HCD_START */
  ...
  while (1)
  {
    ...
    if ((ux_dev_info.Dev_state == Device_connected) && (ux_dev_info.Device_Type == CDC_ACM_Device))
    {
      ...
      else
      {
        ...
        /*reset the app state to Idle */
        ux_app_state = App_Idle;

      }
    }
    if (ux_dev_info.Dev_state == Device_disconnected)
      {
