...
/* USER CODE BEGIN PV */
    ...
    /* Buffers for FileX FX_MEDIA sector cache. this should be 32-Bytes
aligned to avoid cache maintenance issues */
uint32_t media_memory_sd[DEFAULT_SECTOR_SIZE / sizeof(uint32_t)];
uint32_t media_memory_ospi[DEFAULT_SECTOR_SIZE / sizeof(uint32_t)];
...
VOID fx_thread_two_entry(ULONG thread_input)
{
  ...
  status =  fx_media_format(&nor_flash_disk,
                            ...
                            (UCHAR *) media_memory_ospi,       // Media buffer pointer
  ...
  status =  fx_media_open(&nor_flash_disk, "FX_LX_NOR_DISK", fx_stm32_levelx_nor_driver,(VOID*)LX_NOR_OSPI_DRIVER_ID , (VOID *) media_memory_ospi, DEFAULT_MEDIA_BUF_LENGTH);
  ...
  /* Open the test file.  */
  status =  fx_file_open(&nor_flash_disk, &fx_file_ospi, "STM32.TXT", FX_OPEN_FOR_WRITE);
  ...
  if (status != FX_SUCCESS)
  {
   ...
   /* Open the test file.  */
  status =  fx_file_open(&nor_flash_disk, &fx_file_ospi, "STM32.TXT", FX_OPEN_FOR_READ);
